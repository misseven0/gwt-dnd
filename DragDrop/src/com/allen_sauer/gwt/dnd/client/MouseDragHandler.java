 ...
 */
class MouseDragHandler implements MouseMoveHandler, MouseDownHandler, MouseUpHandler, TouchStartHandler, TouchMoveHandler,
		TouchEndHandler, TouchCancelHandler
{
	...
	public void onMouseMove(MouseMoveEvent event)
	{
		...
		} else
		{
			if (mouseDownWidget != null)
			{
				if (Math.max(Math.abs(x - mouseDownPageOffsetX), Math.abs(y - mouseDownPageOffsetY)) >= context.dragController
	...
	private void synthesizeAsyncTouchEnd(TouchEndEvent event)
	{
		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand()
		{
			...
			public void execute()
			{
				...
				elem.dispatchEvent(DOMUtil.createTouchEndEvent(bubbles, cancelable, detail, ctrlKey, altKey, shiftKey, metaKey,
						changedTouches));
			}
		});
	}
	...
	private void synthesizeAsyncMouseUp(MouseUpEvent event)
	{
		...
		NativeEvent n = event.getNativeEvent();
		// One click, see
		// https://developer.mozilla.org/en-US/docs/DOM/event.detail
		final int detail = 1;
		...
		Scheduler.get().scheduleDeferred(new ScheduledCommand()
		{
			...
			{
				// TODO determine if we need to set additional event properties
				elem.dispatchEvent(Document.get().createMouseUpEvent(detail, screenX, screenY, clientX, clientY, ctrlKey, altKey,
	...
	public void onTouchMove(TouchMoveEvent event)
	{
		...
		} else
		{
			if (mouseDownWidget != null)
			{
				if (Math.max(Math.abs(x - mouseDownOffsetX), Math.abs(y - mouseDownOffsetY)) >= context.dragController
						.getBehaviorDragStartSensitivity())
				{
					if (!context.selectedWidgets.contains(context.draggable))
					{
