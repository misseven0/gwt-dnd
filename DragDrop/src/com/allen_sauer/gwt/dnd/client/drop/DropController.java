...
public interface DropController
{
	 ...
	 * This occurs when the widget area and the drop target area intersect and
	 * there are no drop targets which are descendants of our drop target which
	 * also intersect with the widget. If there are, the widget engages with the
	...
	/**
	 * Called when the reference widget stops engaging our drop target by
	 * leaving the area of the page occupied by our drop target, or after
	...
	/**
	 * Called with each mouse movement while the reference widget is engaging
	 * our drop target. {@link #onEnter(DragContext)} is called before this
	 * method is called.
	 * 
	 * @see #onEnter(DragContext)
	 * @see #onLeave(DragContext)
